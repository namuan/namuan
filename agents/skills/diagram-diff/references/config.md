# Correcting the map: `.github/diagram-map.yml`

The generated document is regenerated on every run, so editing it is pointless. Corrections live in `.github/diagram-map.yml`, an overlay applied over fresh inference every time. Inference never writes back into this file, which is why a correction keeps holding as the code moves.
...
Every field except `schemaVersion` is optional, and the file itself is optional. The shape is strict: unknown keys are rejected.
...
Walk each selector against the file paths in the document and apply the overlay before rendering.

When you render, watch for corrections that changed nothing about the document they were applied to. That is a map that has drifted out of date, usually because the file a selector named has moved or gone. It is not an error, but it is worth fixing: a correction that matches nothing is a correction nobody is getting.
