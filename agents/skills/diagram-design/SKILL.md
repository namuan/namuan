...
- [ ] If animated, does the complete static/no-JS frame work, does reduced motion hide/disable playback, and is the controller copied verbatim from `template-motion.html`? Also run the bundled `python3 <skill-dir>/scripts/verify-motion.py path/to/generated.html` and `python3 <skill-dir>/scripts/lint-skin.py path/to/generated.html`; manually check print and static-query states on top of the self-check.
