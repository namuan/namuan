...
`<skill-dir>` is this skill's directory; resolve its bundled `scripts/drawio_extract.py` relative to the skill before running the command. The same directory contains the bundled import fixtures used in the worked example.
...
[`assets/example-import-drawio.html`](../assets/example-import-drawio.html) is the output of this procedure run on `<skill-dir>/scripts/fixtures/sample-architecture.drawio` (12 nodes, 8 edges, 2 container groups) at `format=html`, `size=doc-inline`, `detail=balanced`, `audience=mixed`.
