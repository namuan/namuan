...
## 12. Worked example — extended process variant

The extended process variant is fully described by the following inputs. It is an input specification rather than a shipped asset; every coordinate in a rendered SVG is derivable from this block via §2 + §3 + §4. This is the canonical proof that the parametric contract works end-to-end.
